...
def train(model, data_t, test_t, args, device, dtype, kap, eval_args, data_mean, data_std):
    ...
    for step in range(args.steps):
        _N = args.sim_z.shape[0] if getattr(args, "direct_sim", 0) else len(data_t)
        idx0 = rng.integers(0, _N, args.batch)
        x0 = data_t[idx0]
        ...
        if getattr(args, "direct_sim", 0):
            ...
            gi = ((torch.log(s_fwd) - _lo) / (_hi - _lo) * args.sim_s.numel()).long().clamp(0, args.sim_s.numel() - 1)
            rows = torch.as_tensor(idx0, device=s_fwd.device) % args.sim_z.shape[0]
            eps_fwd = (args.sim_z[rows, gi] - x0) / (2 * s_fwd).sqrt().unsqueeze(1)
        else:
...
def main():
    ...
    ap.add_argument("--direct_sim", type=int, default=0, help="R1对照: 1=用 x-系直接模拟(欧拉-丸山)替换闭式前向")
    ap.add_argument("--sim_n", type=int, default=8192, help="模拟轨迹数")
    ...
    if args.direct_sim:
        ...
        print(f"[dsim] 模拟表就绪: {args.sim_z.shape} (欧拉-丸山 dt={args.sim_dt})", flush=True)
    ...
    json.dump(dict(run_id=run_id, steps=args.steps,
                   ...
                   runtime=dict(train=t_train, sample=t_sample),
                   args={k: v for k, v in vars(args).items() if k not in ("sim_z", "sim_s")}),
