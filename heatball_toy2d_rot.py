...
def make_dataset(name):
    ...
    if name == "ring":
        return Ring2D()
    if name == "iso_gauss":
        return GMM2D([1.0], [(0, 0)], [np.eye(2)], name)
    if name == "aniso_gauss":
        return GMM2D([1.0], [(0, 0)], [np.diag([2.25, 0.09])], name)
...
def main():
    ...
    ap.add_argument("--dataset", default="sparse_dense_gmm",
                    choices=["sparse_dense_gmm", "asym_gmm", "ring", "iso_gauss", "aniso_gauss"])
    ...
    # args.omega = 0.0            # 旋转强度 rho = omega (gamma=1); rho 扫描改这里
    # args.rot_profile = "osc"    # const / osc / ramp; 注意: rho 扫描请用 "const"
    # args.rot_k = 3.0            # osc/ramp 轮廓的频率 k
    # args.outdir = "out_base"    # 每次运行必须改成不同目录, 否则 metrics.json/model.pt 互相覆盖!
    # ==============================================================================
